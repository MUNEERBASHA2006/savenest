  ...
  useEffect(() => {
    async function loadAdvice() {
      try {
        ...
        if (!data) {
          setAiAdvice(
            'Start adding transactions to receive AI insights.'
          );
          return;
        }
        ...
        setAiAdvice('Unable to load AI insights.');
      }
    }
    ...
    <div className="space-y-xl">
      ...
      >
        <div className="flex items-center gap-sm">
          ...
          <span className="text-body-xs font-bold text-secondary uppercase tracking-wider">
        ...
        >
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-md mb-xl">
            ...
            <div className="flex items-center gap-sm bg-secondary-container/30 px-md py-sm rounded-lg">
              ...
              <span className="text-data-mono text-on-secondary-container text-sm">
                Real-time Sync
              </span>
            </div>
          </div>
      ...
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-white rounded-xl p-lg border border-surface-container shadow-[0_4px_12px_rgba(26,43,60,0.05)]"
      >
      ...
      <section className="space-y-md">
        ...
        {targets.length === 0 ? (
          ...
          >
            ...
            <div className="space-y-xs">
              <h4 className="text-headline-sm text-primary">
                No Active Targets
              </h4>

              <p className="text-body-sm text-on-surface-variant max-w-xs mx-auto">
                Setting your first target is the secret to
                financial discipline. What are we building
                today?
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-lg">
            {targets.slice(0, 2).map((target) => (
              <TargetCard
                ...
                progress={Math.round(
                  (target.saved / target.amount) * 100
                )}
            ...
            >
              ...
              <p className="text-label-caps text-on-surface-variant font-bold">
                New Target
              </p>
            </div>
          </div>
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-lg">
        ...
        <span className="text-data-mono text-secondary font-bold">
          {progress}%
        </span>
      </div>
